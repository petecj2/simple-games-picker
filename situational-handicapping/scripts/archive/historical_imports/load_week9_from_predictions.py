#!/usr/bin/env python3
"""
Load Week 9 games from week9_predictions.md file.

Parses the markdown table and populates the games table with the actual
Week 9 schedule for situational analysis.

Usage:
    python load_week9_from_predictions.py
"""
...
PREDICTIONS_FILE = DATA_DIR / "week9_predictions.md"
...
def parse_date_time(date_str, time_str):
    ...
    # Date format: "Thu 10/30" or "Sun 11/2"
    ...
    # Determine year (Week 9 is in October/November)
    year = 2025
...
def load_week9_games():
    """Load Week 9 games from predictions markdown file."""
    ...
    for game in games:
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, [2025, 9, game_date, game_time, home_id, away_id, ''])
    ...
    cursor.execute("""
        SELECT COUNT(*) as total,
               COUNT(CASE WHEN game_date = '2025-10-30' THEN 1 END) as thursday,
               COUNT(CASE WHEN game_date = '2025-11-02' THEN 1 END) as sunday,
               COUNT(CASE WHEN game_date = '2025-11-03' THEN 1 END) as monday
        FROM games
